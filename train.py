def main(cfg):
    ...
    if cfg['optimizer_type'] == 'sgd':
        ...
        optimizer = optim.RMSprop(net.parameters(), lr=cfg['init_lr'], momentum=cfg['momentum'],
                                  weight_decay=cfg['weight_decay'], alpha=0.9, eps=cfg['eps'])
    else:
        raise ValueError \
    ...
    if cfg['load_from'] != "":
        ...
        if os.path.exists(cfg['load_from']):
            load_train_weight(net, cfg['load_from'], optimizer, scheduler, scaler, net_ema, cfg)
        else:
    ...
    checkpoint = {'epoch': cfg['epochs'], 'net': net.state_dict(), 'optimizer': optimizer.state_dict(),
                  'scheduler': scheduler.state_dict()}
    if scaler:
        checkpoint["scaler"] = scaler.state_dict()
    if net_ema:
...
if __name__ == '__main__':
    ...
    if cfg['device'] == 'cuda':
        assert len(cfg['gpu_ids'].split(',')) == 1
